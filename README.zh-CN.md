...
把试听样本放入音色的 `samples/` 后，显式确认公开展示权利并生成静态预览：

```bash
moon-media assets voices approve moon-reader-v1 \
  --name "Moon Reader" \
...
# output/voice-catalog/index.html
```

公开目录只复制已指定的试听样本和去身份化元数据，不包含 `reference.wav`、
参考逐字稿、原始路径或内部生成参数。
